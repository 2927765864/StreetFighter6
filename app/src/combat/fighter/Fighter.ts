...
export class Fighter {
  ...
  setIdleWalk(clip: 'idle' | 'walk' | 'crouch'): void {
    ...
    this.phase = clip === 'walk' ? 'walk' : clip === 'crouch' ? 'crouch' : 'idle';
    if (clip !== 'walk') {
      this.clearLoco();
      this.clipId =
        clip === 'crouch' ? 'crouch' : 'idle';
      this.animRole = 'main';
    }
  }
  ...
  applyWalkState(ws: WalkState): void {
    ...
    if (ws.locoPhase === 'none') {
      this.phase = this.phase === 'crouch' ? 'crouch' : 'idle';
      this.clipId = this.phase === 'crouch' ? 'crouch' : 'idle';
      this.animRole = 'main';
    } else {
      this.phase = 'walk';
