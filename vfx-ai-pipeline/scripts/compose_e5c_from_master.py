#!/usr/bin/env python3
"""E5-c narrow-long fiber smoke: locked master = 1-based frame 8 → 28 frames.

f1: uniform 0.5 scale, alpha 0.2.
f1→f8: ease-out scale 0.5→1, alpha 0.2→1, large move right (arrive at master).
f9→f28: slow horizontal stretch, slow right, fade to 0.
"""
...
OUT = ROOT / "runs" / "hit_ref_v1" / "E5c_narrow_long_fiber_smoke"
...
SX_F8, SY_F8 = 1.0, 1.0
SX_F28, SY_F28 = 1.42, 1.0
ALPHA_F1 = 0.20

# Raw path relative to master. +x right.
VX_PRE = 0.85  # f1→f8 fast right (arrive at 0)
VX_POST = 0.14  # f9→f28 slow right (weaker than pre so master stays large)
...
def axes_scale(i: int) -> tuple[float, float]:
    ...
    return lerp(SX_F8, SX_F28, u), SY_F8
...
def fit_motion(
    ...
    for amp in np.linspace(target_amp, cell * 0.10, 32):
        ...
        if ok_base is not None and ok_base > 0.04:
            ...
            break
    if best is None:
        raise RuntimeError("could not fit E5-c trajectory into cell")
...
def main() -> None:
    ...
    meta = {
        "element": "E5c_narrow_long_fiber_smoke",
