#!/usr/bin/env python3
"""E8-b2 arc smoke: E8-b1 mirrored vertically (opposite arc facing) → same 12-frame transform.

f4 = master_01 @ 1.0
f1 = master_01 @ 0.7 + H-compress; f1→f4 scale up + H-stretch + up
f8 = master_02 @ 1.2; f5→f8 enlarge + up
f9→f12 H-stretch + fade + up
All frames drift upward (opposite of E8-b1).
"""
...
OUT = ROOT / "runs" / "hit_ref_v1" / "E8b2_arc_smoke"
...
# Opposite of E8-b1: negative = upward (screen +y is down).
DY_PER_FRAME = -0.11
...
def raw_offset(i: int) -> tuple[float, float]:
    # Relative to frame 4 (master_01 natural). All frames move up (vs E8-b1 down).
...
def fit_motion(
    ...
    for amp in np.linspace(target_amp, cell * 0.10, 24):
        ...
        if ok_base is not None and ok_base > 0.04:
            ...
            break
    if best is None:
        raise RuntimeError("could not fit E8b2 trajectory into cell")
...
def main() -> None:
    ...
    meta = {
        ...
        "motion": (
            ...
            "f5–f8 enlarge→1.2 + up; f9–f12 H-stretch + fade + up "
            "(vertical flip masters + upward drift vs E8-b1)"
        ),
    }
    (OUT / "pipeline-meta.json").write_text(json.dumps(meta, indent=2) + "\n")
    (OUT / "prompt-used.txt").write_text(
        "E8b2 12f from E8-b1 masters flipped top-bottom. f4=m1, f8=m2@1.2. "
        "f3–f6 overlapping crossfade, m2 enters at 0.9. "
        "Upward drift entire clip (opposite of E8-b1 down); same scale/stretch/fade.\n"
    )
