#!/usr/bin/env python3
"""E4-c wide-short fiber smoke: locked master = 1-based frame 1 → 18 frames.

f1: master as-is (sx, sy) = (1, 1).
f1→f10: slight uniform grow + extra vertical squash (grow > squash),
         large move right.
f11→f18: horizontal stretch, fade out, slow right.
"""
...
OUT = ROOT / "runs" / "hit_ref_v1" / "E4c_wide_short_fiber_smoke"
PUBLIC = ROOT.parent / "app" / "public" / "vfx" / "hit_ref_v1" / "E4c_wide_short_fiber_smoke"
...
GROW_F10 = 1.18
V_COMPRESS = 0.88  # applied after grow; 18% grow > 12% squash
H_STRETCH_F18 = 1.34  # extra sx from f10 to f18

SX_F1, SY_F1 = 1.0, 1.0
SX_F10, SY_F10 = GROW_F10, GROW_F10 * V_COMPRESS
SX_F18, SY_F18 = SX_F10 * H_STRETCH_F18, SY_F10

# Raw path. +x right. Pre is large, post is slow.
VX_PRE = 1.00
VX_POST = 0.22


def chroma_alpha(im: Image.Image) -> Image.Image:
    """Unmix magenta key so thin smoke stays, RGB has no purple fringe."""
...
def axes_scale(i: int) -> tuple[float, float]:
    if i <= F10:
        u = ease_out(i / F10) if F10 else 1.0
        return lerp(SX_F1, SX_F10, u), lerp(SY_F1, SY_F10, u)
    u = (i - F10) / (FRAMES - 1 - F10)
    u = ease_out(u)
    return lerp(SX_F10, SX_F18, u), lerp(SY_F10, SY_F18, u)


def raw_offset(i: int) -> tuple[float, float]:
    if i <= F10:
        return VX_PRE * (i / F10), 0.0
    return VX_PRE + VX_POST * ((i - F10) / (FRAMES - 1 - F10)), 0.0
...
def fit_motion(
    ...
    """f1→f10 center travel = 2 × f1 sprite width; f11→f18 keep slow right."""
    raw = [raw_offset(i)[0] for i in range(FRAMES)]
    ...
    def layout(base: float) -> list[tuple[float, float, float, float]] | None:
        ...
        for i in range(FRAMES):
            ...
            nw = mw * base * sx
            nh = mh * base * sy
            cx = cx0 + travel_f10 * (raw[i] / VX_PRE)
            x = cx - nw / 2.0
            y = (cell - nh) / 2.0
    ...
    if best_base is None or best_layout is None:
        raise RuntimeError("could not fit E4-c 2-width travel into cell")
...
def main() -> None:
    ...
    meta = {
        "element": "E4c_wide_short_fiber_smoke",
        ...
        "fps": FPS,
        "cell": CELL,
        "fade_from_frame_1based": 11,
        "base_fit_scale": round(base, 5),
        "axes": {
            "f1": [SX_F1, SY_F1],
            "f10": [round(SX_F10, 4), round(SY_F10, 4)],
            "f18": [round(SX_F18, 4), round(SY_F18, 4)],
        },
        "grow_f10": GROW_F10,
        "v_compress": V_COMPRESS,
        "h_stretch_f18": H_STRETCH_F18,
        "motion": (
            "f1 = locked master. f1–f10 slight grow + V-squash "
            "(grow > squash) + right travel = 2× f1 sprite width. "
            "f11–f18 H-stretch + fade + slow right."
        ),
    }
    (OUT / "pipeline-meta.json").write_text(json.dumps(meta, indent=2) + "\n")
    (OUT / "prompt-used.txt").write_text(
        "E4-c master = frame 1 of 18 (user-locked). PIL-only: "
        "f1-10 grow 1.18 + V-compress 0.88 + right travel 2× sprite width; "
        "f11-18 H-stretch 1.34, fade, slow right.\n"
    )

    print(f"wrote {FRAMES} frames → {OUT}")
    print(f"  master {mw}x{mh}, base_fit={base:.4f}")
    w1 = mw * base * motion[0][0]
    w10 = mw * base * motion[F10][0]
    cx = []
    for i, (sx, sy, dx, dy, a) in enumerate(motion):
        nw = mw * base * sx
        cx.append((CELL - nw) / 2 + dx)
    print(
        f"  f1 width={w1:.1f}  f1→f10 center travel={cx[F10] - cx[0]:.1f} "
        f"({(cx[F10] - cx[0]) / w1:.2f}× f1 width)"
    )
    for i in (0, 4, 9, 10, 13, 17):
