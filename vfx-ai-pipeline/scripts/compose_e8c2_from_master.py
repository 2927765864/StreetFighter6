#!/usr/bin/env python3
"""E8-c2 right-spread smoke: one locked master (CCW 90°) → 28-frame transform.
...
OUT = ROOT / "runs" / "hit_ref_v1" / "E8c2_right_spread_smoke"
...
def remat_ff00ff(im: Image.Image, thr: float = 55.0) -> Image.Image:
    """Grok Imagine often delivers a dull magenta; retarget the field to #FF00FF."""
    ...
    dist = np.linalg.norm(arr - key, axis=-1)
    arr[dist <= thr] = (255.0, 0.0, 255.0)
    return Image.fromarray(np.asarray(np.clip(arr, 0, 255), dtype=np.uint8))
...
def fit_motion(
    ...
    for amp in np.linspace(target_amp, cell * 0.10, 32):
        ...
        if ok_base is not None and ok_base > 0.04:
            ...
            break
    if best is None:
        raise RuntimeError("could not fit E8-c2 trajectory into cell")
...
def main() -> None:
    ...
    rematted = remat_ff00ff(Image.open(MASTER))
    rematted.save(OUT / "master_ccw90_keyed.jpg", quality=95)
    keyed = chroma_alpha(rematted)
    ...
    meta = {
        "element": "E8c2_right_spread_smoke",
    ...
    (OUT / "prompt-used.txt").write_text(
        "E8-c2 CCW90 master = frame 10 of 28. PIL-only sequence: "
